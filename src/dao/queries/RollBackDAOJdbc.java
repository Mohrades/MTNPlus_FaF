	...
	public void saveOneRollBack(RollBack rollBack) {
		try {
			if(rollBack.getId() == 0) {
				...
				getJdbcTemplate().update("INSERT INTO MTN_PLUS_ROLLBACK_EBA (STEP,VALUE,ANUMBER,BNUMBER,ERROR_TIME,ERROR_TIME_INDEX) VALUES(" + rollBack.getStep() + "," + rollBack.getValue() + ",'" + rollBack.getAnumber() + "','" + rollBack.getBnumber() + "',TIMESTAMP '" + dateFormat.format(now) + "'," + Integer.parseInt(new SimpleDateFormat("yyyyMMdd").format(now)) + ")");			
			}
